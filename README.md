...
- Fires remember who lit them, one player at a time. The first player to start a fire while nothing else on the server is burning is recorded, and that player id stays with every fire on the server, however far it spreads, until all of it is out. Until then a fire lit anywhere else, by another player, lightning or a creature, is booked to the same player. A fire that lightning or a creature starts on a quiet server records nobody, and so do fires started with the admin commands. That id is saved to disk with the rest of the fire state, in `firefront_fires_<world id>.txt` in the server's `worlds_local` folder, so it survives a restart; with `persistfires` off nothing is written. Nothing surfaces in game yet: it feeds a companion mod's reputation system.
