...
| | |
|---|---|
| Valheim | 1.0.16 (built and checked against it; last tested in game on 1.0.15; older builds are not supported) |
...
- Fires remember who lit them, one fire at a time. The player whose hit sets an object alight is recorded on it, and fire that spreads, to other objects or to the ground around them, keeps the igniter of the fire it came from — so two fires burning at once on the same server are attributed separately. A fire that lightning or a creature starts, or one lit with the admin commands, records nobody. That id is saved to disk with the rest of the fire state, in `firefront_fires_<world id>.txt` in the server's `worlds_local` folder, so it survives a restart; a save from an older version restores with the igniter unknown, and with `persistfires` off nothing is written. Nothing surfaces in game yet: a companion mod can read it for its reputation system, fire by fire rather than one player for the whole server.
...
`ignite`, `startfire` and `clearfires` run on the server no matter where you type them, only for players on the server's own admin list, and the reply comes back to your console prefixed `[server]`. `fireset` changes the copy on your own machine and asks the server, which applies it only for its admins, whether typed or moved from a config manager; its answer comes back the same way. `firestatus` asks the server too and needs no admin. `stopfire` puts out only the one thing under your crosshair, unlike `G`, which also clears the fire around you. It checks the admin flag your own game holds, which can say no even to a real admin on a dedicated server; `G` needs no admin. `firedebug` turns verbose logging on or off on your own machine; on a client connected to a server it also sends the new value to the server like `fireset debug`, which the server applies only for its admins.
...
- Change `groundcellsize` only while no ground fire burns. Changing it on the server (or in your own game, in single player or when hosting) while ground fire burns moves the burning cells to the new grid, and a fire save written then keeps the mixed cells.
