...
Moved after 1.0, in the order written below: from 0.24, the server-to-client config broadcast, per-payload wire versions and the sidecar downgrade story; all of 0.25 to 0.28; and the rest of 0.29 (dead-side key descriptions, the admin guide, localisation tokens, the compatibility position, CI and checksums, the tester pipeline). Found in the README pass and also moved after 1.0: per-fire arson attribution. The server keeps one igniter id for the whole world (the first player to light a fire while nothing burns, cleared when every fire is out), and that one value is what is saved and what the companion mod reads; each fire event already records its own igniter (`FireEvent.IgniterPlayerId`) but nothing reads it. The README states the current behaviour. Condition 2 of "What 1.0 means" is therefore met by its last clause only at 1.0: mismatched versions are told so at connect; the per-key descriptions and preset override logs come in 1.x.
