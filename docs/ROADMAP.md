...
Moved after 1.0, in the order written below: from 0.24, the server-to-client config broadcast, per-payload wire versions and the sidecar downgrade story; all of 0.25 to 0.28; and the rest of 0.29 (dead-side key descriptions, the admin guide, localisation tokens, the compatibility position, CI and checksums, the tester pipeline). Found in the README pass and also moved after 1.0, then done at 1.0.2: per-fire arson attribution. At 1.0 the server kept one igniter id for the whole world (the first player to light a fire while nothing burns, cleared when every fire is out), and that one value was what was saved and what the companion mod read. 1.0.2 gives every burning object and ground cell its own igniter, inherited by whatever it spreads to and saved with it; a companion mod can read it per fire through `FireManager.CollectActiveFiresWithIgniters` / `FireIgniterNear`. The old map-wide value (`CurrentFireIgniterPlayerId`) is unchanged, so older companion builds keep working. The README states the current behaviour. Condition 2 of "What 1.0 means" is therefore met by its last clause only at 1.0: mismatched versions are told so at connect; the per-key descriptions and preset override logs come in 1.x.
...
- ~~Restored ground-fire cells get a `FireBurnZone`~~ Already done in 0.21.9 (`9900edf`): the restore path queues each cell for `UpgradeDarkGroundCells`, which attaches the zone. The handoff item (docs/HANDOFF.md:586-589) predates the fix.
