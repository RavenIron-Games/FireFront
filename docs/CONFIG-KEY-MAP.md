...
| Controls | `ExtinguishGroundRadius` | both | `extinguishradius` |  | server copy is dead |
...
| Items | `DousingBombRadius` | both | `dousingradius` |  | server copy is dead |
...
- `GroundCellSize`: Real 'both, for different purposes' key: the server's own copy sizes the authoritative ignition/spread grid (cell keys, firebreak sampling, damage-zone radius, the vertical burn band); the client's own copy independently sizes and positions the scorch decals it draws locally from synced ground-fire deltas. A client running a different GroundCellSize than the server would only misalign its own decorative decals with the real grid, not the simulation itself.
